...
class EtapPvSldService:
        ...
        for i in range(len(df1)):
            ...
            if df1.at[i, 'From_Device_Type'] == 'Gen':
                 ...
                 for s in suffixes:
                     ...
                     all_layout.append(f"""<PVArray ID="{df1.at[i, gen_key]}" LocX="{df1.at[i, gen_locX]}" LocY="{df1.at[i, gen_locY]}" Orientation="90"/>""")
    ...
    def _generate_all_connections(self, df_elements: pd.DataFrame):
        ...
        for _, row in df_ids.iterrows():
            ...
            for label in valid_labels:
                pv_col = f"Gen_{label}_ID"
                dc_bus_col = f"DC_Bus_{label}_ID"
                
                pv_id = row.get(pv_col)
                dc_bus_id = row.get(dc_bus_col)
                
                if pd.isna(pv_id) or pd.isna(dc_bus_id): continue
                
                pv_id = str(pv_id)
                dc_bus_id = str(dc_bus_id)
                
                pv_info = id_map.get(pv_id, {})
                dc_info = id_map.get(dc_bus_id, {})
                
                connections.append(f'''<CONNECT FromElement="PVArray" FromID="{pv_id}" FromIID="{pv_info.get("IID", "0")}" FromExtRef="0" FromPin="0" FromCloneGuid="" ToElement="{dc_info.get("ElementType", "DCBUS")}" ToID="{dc_bus_id}" ToIID="{dc_info.get("IID", "0")}" ToExtRef="0" ToPin="1" ToCloneGuid=""/>''')
